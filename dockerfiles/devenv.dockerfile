...
# Stage 3: Production build (original behavior)
FROM machnet_build_base AS machnet

WORKDIR /home/vj2267/machnet

# Copy Machnet files
COPY --chown=vj2267:vj2267 . .
...
# Clean up unneeded static libraries to save space
RUN find /usr/local/lib64 -name "*/librte_*.a" | xargs rm -f

CMD /home/vj2267/machnet/release_build/src/apps/machnet/machnet --config_json /var/run/machnet/local_config.json
